...
def get_user_permissions(request):
    # Permissions are resolved the same way under both auth providers: the
    # Hanko middleware maps the session onto a real Django User, so per-user
    # and group permissions still apply. Only the displayed username differs
    # (Hanko exposes an OSM username or an email, not a Django username).
    user = request.user
