...
class HDXExportSet(object):
    ...
    def hdx_note(self, theme):
        ...
        return self._extra_notes + MARKDOWN.format(
            region=self._name, columns=columns, filter_str=filter_str
        )
    ...
    def datasets(
        ...
        d = []
        updated_by_script = (
            f'HOT Export Tool ({datetime.now().strftime("%Y-%m-%dT%H:%M:%S")})'
        )
        for theme in self._mapping.themes:
            dataset = Dataset()
            ...
            dataset["name"] = "{0}_{1}".format(
                self._dataset_prefix, slugify(theme.name)
            )
            dataset["title"] = "{0} {1} (OpenStreetMap Export)".format(
            ...
            if "hdx" in theme.extra:
                ...
                if "tags" in theme.extra["hdx"]:
                    tags = [
                        tag.strip() for tag in theme.extra["hdx"]["tags"].split(",")
                    ]
            ...
            for f in files:
                if isinstance(f, dict):  # it is coming from galaxy
                    if f["theme"] == theme.name:
                        file_name = f"{f['file_name']}_{f['output_name']}"  # only one part: the zip file
                        resources.append(
                            {
