...
# OAuth2 provider — mounted under both auth providers. These routes are the
# public API's authorization server (third parties create their own client
# applications and mint bearer tokens here), not part of user login, so they
# outlive the switch to Hanko. DOT's views only need request.user, which the
# Hanko middleware populates. The Help > API page links to
# /o/applications/ and /o/authorized_tokens/ directly.
urlpatterns += [
