...
class OSMParser(object):
    ...
    def __init__(self, sqlite=None, stage_dir=None, schema=None, categorized_tags=None, debug=None):
        self.path = os.path.dirname(os.path.realpath(__file__))
        self.stage_dir = stage_dir
        self.sqlite = sqlite
        if not os.path.exists(self.sqlite):
            raise IOError('Cannot find SQlite data for this task.')
        self.schema = schema
        self.debug = debug
        """
            OGR Command to run.
        ...
        try:    
            ...
            logger.error("Error creating thematic sqlite file.")
            raise Error("Error creating thematic sqlite file", e)
    ...
    def generate_thematic_sql(self, ):
        
    
    def create_thematic_schema(self, ):
        # update the spatialite schema
        self.update_sql = Template("spatialite $sqlite < $thematic_sql")
        sql_cmd = self.update_sql.safe_substitute({'sqlite': self.thematic_sqlite,
                            'thematic_sql': self.thematic_sql})
        if(self.debug):
    ...
    def generate_thematic_shapefile(self, ):
        pass
