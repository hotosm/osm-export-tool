...
def login(request):
    if settings.AUTH_PROVIDER == 'hanko':
        # Only follow redirects back into this site: ?next=https://evil.example.com
        # would otherwise bounce the user off-site right after logging in. The
        # login service validates return_to too, but don't rely on it.
        ...
        if is_hanko_authenticated(request):
            # Valid Hanko session. Send the user where they were headed if they
            # already have an account mapped; otherwise finish onboarding first,
            # since redirecting an unmapped user to a @login_required page (e.g.
            # /o/authorize) would bounce them straight back here.
            if get_mapped_django_user(request):
                return redirect(next_url)
            params = urlencode({
                'onboarding': APP_NAME,
                'return_to': request.build_absolute_uri(next_url),
            })
            return redirect(f"{hanko_url}/app?{params}")

        # urlencode the return_to: an OAuth2 /o/authorize target carries its own
        # query string, and an unescaped & would be parsed as a parameter of the
        # login URL, truncating where the user comes back to.
        params = urlencode({'return_to': request.build_absolute_uri(next_url)})
        return redirect(f"{hanko_url}/app?{params}")
