    ...
    def run_duration(self):
        if self.started_at and self.finished_at:
            return time.strftime(
                "%H:%M:%S",
                time.gmtime((self.finished_at - self.started_at).total_seconds()),
            )
        return None

    @property
    def elapsed_time(self):
        return (self.finished_at or timezone.now()) - (
            self.started_at or self.created_at
        )

    @property
    def size(self):
        return sum(map(lambda task: task.filesize_bytes or 0, self.tasks.all()))
            ...
            if valid == True:
                ...
                try:
                    filesize_bytes = os.path.getsize(
                        os.path.join(
                            settings.EXPORT_DOWNLOAD_ROOT, str(self.run.uid), fname
                        ).encode("utf-8")
                    )
                except Exception:
                    filesize_bytes = 0
                download_url = os.path.join(
                    settings.EXPORT_MEDIA_ROOT, str(self.run.uid), fname
                )
...
class ExportRunAdmin(admin.ModelAdmin, ExportCsvMixin):
    ...
    def job_ui_link(self, obj):
        return mark_safe(
            f"""<a href="https://{settings.HOSTNAME}/en/v3/exports/{obj.job.uid}" target="_blank">UI Job Link</a>"""
        )

    def job_link(self, obj):
        return mark_safe(
            '<a href="%s">Link to Job</a>'
...
class ExportRunsInline(admin.TabularInline):
    ...
    def change_link(self, obj):
        return mark_safe(
            '<a href="%s">Link</a>'
            % reverse("admin:tasks_exportrun_change", args=(obj.id,))
        )
...
class HDXExportRegionAdmin(admin.ModelAdmin, ExportCsvMixin):
    ...
    list_filter = ("schedule_period", "is_private", "schedule_hour")
    raw_id_fields = ("job",)
    ...
    def edit_link(self, obj):
        return mark_safe(
            f"""<a href="https://{settings.HOSTNAME}/en/v3/hdx/edit/{obj.id}" target="_blank">Edit HDX Job</a>"""
        )
